...
**Update 2026-07-14:** Per Shane's simplicity request, student PINs are now **4 digits** (padded internally with `PIN_SUFFIX` to meet Firebase's 6-char minimum — the suffix constant must stay identical in `firebase-app.js` and `api/reset-pin.js`). Class code kept (once-per-device + roster privacy). Re-verified live: create with 4-digit PIN → sign out → re-sign-in works; test data cleaned up.
